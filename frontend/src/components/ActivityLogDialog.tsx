...
const formatLogDetails = (log: ActivityLog, entityType: string, teams: Team[], departments: Department[]) => {
  ...
  return details.map((detail, index) => {
    ...
    switch (detail.field) {
      case 'status':
        if (entityType === 'USER') {
          fromValue = fromValue ? `'${fromValue}'` : '없음';
          toValue = toValue ? `'${toValue}'` : '없음';
  ...
  useEffect(() => {
    if (open && entityId) {
      ...
      Promise.all([
        ...
        getDepartments(),
        getTeams(),
      ]).then(([fetchedLogs, fetchedDepartments, fetchedTeams]) => {
        setLogs(fetchedLogs);
                  ...
                  <ListItemText
                    ...
                    secondary={
                      <>
                        ...
                        {formatLogDetails(log, entityType, teams, departments)}
                      </>
                    }
                  />
                </ListItem>
