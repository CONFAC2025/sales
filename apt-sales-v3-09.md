...
## 2025년 10월 2일: 동적 권한 관리 시스템 구축 계획

현재 앱은 안정적으로 배포되었으나, 모든 기능이 정상 작동하려면 사용자 직급별 권한 문제가 해결되어야 합니다. 사용자의 요청에 따라, 하드코딩된 권한 대신 최고관리자가 각 직급의 권한을 직접 제어할 수 있는 동적 권한 관리 시스템을 구축하는 것을 장기적인 목표로 설정합니다.

이를 위해 두 가지 경로를 제안합니다.

### 경로 A: 단기 해결책 (즉시 실행)

현재 `총괄본부장` 계정이 기능에 접근하지 못하는 `403 Forbidden` 오류를 해결하기 위해, 코드에 직접 권한을 추가하여 앱을 즉시 정상화합니다.

- **작업 내용:**
    1. `src/routes/admin.ts` 파일에 `GENERAL_HQ_MANAGER` 직급 추가하여 '사용자 관리' 접근 허용.
    2. `src/routes/chat.ts` 파일에 `GENERAL_HQ_MANAGER` 직급 추가하여 '채팅' 기능 접근 허용.
- **기대 효과:** 앱의 모든 기능이 즉시 정상적으로 작동함.

### 경로 B: 장기 프로젝트 (권한 관리 시스템 구축)

경로 A로 문제를 해결한 후, 장기적으로 유지보수와 확장이 용이한 동적 권한 관리 시스템을 구축합니다.

- **작업 내용:**
    1.  **데이터베이스 설계:** `Role`, `Permission`, `RolePermission` 등 권한 관리를 위한 새로운 테이블 스키마를 `prisma/schema.prisma`에 정의.
    2.  **백엔드 구현:**
        - 모든 API 요청 시, DB에 저장된 권한을 확인하도록 `authMiddleware` 로직을 전면 재작성.
        - 직급별 권한을 조회하고 수정하는 새로운 API 엔드포인트 구현.
    3.  **프론트엔드 구현:**
        - 최고관리자가 직급별 권한을 설정할 수 있는 새로운 관리자 UI 페이지 제작.
